...
function ENT:Initialize()
	...
	-- Get snack machine items.
	self.items = {}

	for name, item in pairs(ix.item.list) do
		if (string.match(name, "chips") or string.match(name, "candy")) then
...
function ENT:CreateSnack()
	...
	ix.item.Spawn(item, position + f*20.5 + r*4 + u*-26, function(item, entity) end)
end
				...
				if (activator:GetChar():GetFaction() == FACTION_MAINTENANCE and activator:KeyDown(IN_SPEED)) then
					...
					if (self:GetStock() == 0 or !activator:GetChar():HasMoney(5)) then
						...
						if !(activator:GetChar():HasMoney(5)) then
							activator:Notify("You need "..ix.currency.Get(5).." to use this snack machine.")
						end
						
						return
					elseif (!activator.nextSnackMachine or curTime >= activator.nextSnackMachine) then
						self:CreateSnack(activator)
						
						activator.nextSnackMachine = curTime + 600
						
						activator:GetChar():TakeMoney(5)
						activator:Notify("You have spent "..ix.currency.Get(5).." on this snack machine.")
					else
						self:SetFlashDuration(3)
					end
				end
