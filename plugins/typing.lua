...
if (CLIENT) then
	...
	function PLUGIN:LoadFonts(font, genericFont)
		...
		surface.CreateFont("ixTypingIndicatorText", {
			font = genericFont,
			size = 128,
			extended = true,
			weight = 1000
	...
	function PLUGIN:PostDrawTranslucentRenderables()
		...
		for _, v in ipairs(player.GetAll()) do
			...
			cam.Start3D2D(self:GetTypingIndicatorPosition(v), Angle(0, angle.y, 90), 0.05)
				surface.SetFont("ixTypingIndicatorText")
				...
				draw.SimpleTextOutlined(text, "ixTypingIndicatorText", 0,
				...
				local x, y = 0, - 20
				ix.util.DrawText("•", x - 40, y, ColorAlpha(textColor, offset1), 1, 1, "ixTypingIndicator", offset1)
				ix.util.DrawText("•", x, y, ColorAlpha(textColor, offset2), 1, 1, "ixTypingIndicator", offset2)
				ix.util.DrawText("•", x + 40, y, ColorAlpha(textColor, offset3), 1, 1, "ixTypingIndicator", offset3) ]]
			cam.End3D2D()
		end
	end
