...
if (CLIENT) then
	...
	function PLUGIN:PrePACEditorOpen(client)
		if (!client:GetCharacter():HasFlags("Z")) then
			return false
		end
	end
end
...
function PLUGIN:pac_CanWearParts(client)
	if (client:GetCharacter() and !client:GetCharacter():HasFlags("Z")) then
		return false
	end
end
	...
	local function AttachPart(client, uniqueID)
		...
		if (pacData) then
			if (itemTable) then
				...
				elseif itemTable.getPacData then
					pacData = itemTable:getPacData(client)
				end
			end
