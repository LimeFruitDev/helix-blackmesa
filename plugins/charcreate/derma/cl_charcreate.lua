...
function PANEL:Init()
	...
	descriptionProceed.DoClick = function()
		if (self:VerifyProgression("description")) then
			...
			if (self.creationData) then
				if (self.creationData["skin"]) then
					self.attributesModel:GetEntity():SetSkin(self.creationData["skin"])
				end

				if (self.creationData.groups) then
					for k, v in pairs(self.creationData["groups"]) do
						self.attributesModel:GetEntity():SetBodygroup(k, v)
					end
				end
			end
		end
	...
	-- heads
	local customizePanel = self.descriptionPanel:Add("DPanel")
	customizePanel:Dock(BOTTOM) -- TODO: don't fill so we can allow other panels
	customizePanel:SetTall(200)
	customizePanel.Paint = function(panel, width, height)
	end

	local scroll = customizePanel:Add("DScrollPanel")
	...
	-- for _, v in pairs(ix.plugin.list["heads"].HeadModels) do
	-- 	local icon = layout:Add("SpawnIcon")
	-- 	icon:SetSize(64, 128)
	-- 	icon:InvalidateLayout(true)
	-- 	icon.DoClick = function(this)
	-- 		-- payload:Set("head", k)
	-- 		self.creationData["head"] = v
	-- 	end
	-- 	icon.PaintOver = function(this, w, h)
	-- 		if (self.creationData["head"] == v) then
	...
	-- local customizeFooter = customizePanel:Add("DPanel")
	-- customizeFooter:Dock(BOTTOM)
	-- customizeFooter:SetTall(75)
	-- customizeFooter.Paint = function(panel, width, height)
	-- end

	-- bodygroups
	-- self.bodygroups = customizeFooter:Add("DButton")
	self.bodygroups = customizePanel:Add("DButton")
	self.bodygroups:Dock(LEFT)
	self.bodygroups:DockMargin( 0, 10, 5, 0 )
	self.bodygroups:SetText("Bodygroups")
	self.bodygroups:SetFont("ixCustomizeButton")
	self.bodygroups:SizeToContents()
	self.bodygroups:SetWide(self.bodygroups:GetWide() * 2)

	self.bodygroups.DoClick = function(button)
		local ent = self.descriptionModel:GetEntity()
		local options = ent:GetBodyGroups()
		local model = self.factionModel:GetModel()
		...
		for k, v in pairs( options ) do
			...
			if table.HasValue(PLUGIN.bodygroupBlacklist, v.name) then
				shouldContinue = true
				if (PLUGIN.bodygroupBlacklistIgnore[model] and table.HasValue(PLUGIN.bodygroupBlacklistIgnore[model], v.name)) then
			...
			if ( v.num == 2 ) then
				...
				local option = submenu:AddOption( PLUGIN:N(v.name), function()
					ent:SetBodygroup(v.id, opposite)

					self.creationData["groups"] = self.creationData["groups"] or {}
					self.creationData["groups"][v.id] = opposite
				end)
				...
				local groups = submenu:AddSubMenu( PLUGIN:N(v.name) )

				for i=1, v.num do

					if not table.HasValue(PLUGIN.bodygroupBlacklist, v.submodels[i-1]) then
						local modelname = "model #" .. i
						if ( v.submodels && v.submodels[ i-1 ] != "" ) then modelname = v.submodels[ i-1 ] end

						local option = groups:AddOption( PLUGIN:N(modelname), function()
							local opposite = i-1
							ent:SetBodygroup(v.id, opposite)
	...
	-- skins
	self.skins = customizePanel:Add("DButton")
	self.skins:Dock(LEFT)
	self.skins:DockMargin( 0, 10, 5, 0 )
	self.skins:SetText("Skin")
	self.skins:SetFont("ixCustomizeButton")
	self.skins:SizeToContents()
	self.skins:SetWide(self.skins:GetWide() * 2)

	self.skins.DoClick = function(button)
		local submenu = DermaMenu()
		local ent = self.descriptionModel:GetEntity()
	...
	-- add label for entry
	local label = self.descriptionPanel:Add("DLabel")
