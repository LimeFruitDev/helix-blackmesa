...
function PANEL:Init()
	...
	self:SetPos(0, 0)
	self:SetLeftOffset(self:GetPadding())
	...
	-- new menu panel
	self.menu = self:Add("ixMenuNew")
	self.menu:SetSize(ScrW(), ScrH())
	self.menu:Center()

	-- tabs
	self.tabs = self.menu.tabs
	self.tabs.buttons = {}

	local characters = self:AddTab("characters")
	...
	-- local close = self:AddTab("return")
	-- close:SetFont("LimeFruit.Fonts.FontAwesomeSolid")
	-- close:SetText("")
	-- close:SetColor(Color(255, 100, 100))
	-- close:Dock(RIGHT)
	...
	if (IsValid(subpanel)) then
		if (!subpanel.bPopulated) then
			...
			if (istable(info) and info.Create) then
				...
				if subpanel.infoPanel then
					local x, y = self:GetStandardSubpanelSize()
					subpanel.infoPanel:SetSize(x - subpanel:GetWide() * 0.4, y)
				end
...
function PANEL:GetOverviewInfo(origin, angles, fov)
	...
	-- forward.z = 0
	forward.x = forward.x * 0.6
	forward.y = forward.y * 0.6
	forward.z = 15
...
function PANEL:GetStandardSubpanelPosition()
	return 0, ScrH() * 0.15
end

function PANEL:GetStandardSubpanelSize()
	return ScrW() * 0.9, ScrH() * 0.85 - self:GetPadding()
end

function PANEL:AddTab(name)
	local button = self.menu.tabs:Add("LimeFruit.UI.TabButton")
	button:SetText(string.FirstToUpper(L(name)))
...
function PANEL:SetupTab(name, info, sectionParent)
	...
	if (sectionParent) then
		...
		subpanel:SetSize(self:GetStandardSubpanelSize())
		subpanel:SetPos(self:GetStandardSubpanelPosition())
	...
	button.OnSelected = function()
		for _, tab in pairs(self.menu.tabs:GetChildren()) do
			if tab != button then
	...
	if (bTable and info.PopulateTabButton and name != "settings") then
...
function PANEL:PopulateTabs()
	...
	local totalWidth = 0
	for _, tab in ipairs(self.menu.tabs:GetChildren()) do
		totalWidth = totalWidth + tab:GetWide()
	end

	self.menu.tabs:SetWidth(totalWidth)
	self.menu.tabs:SetPos(ScrW()/2 - self.menu.tabs:GetWide()/2, 0)
...
function PANEL:Paint(width, height)
	...
	ix.util.DrawBlur(self, alphaFraction * 15, nil, self.currentBlur * 200)
